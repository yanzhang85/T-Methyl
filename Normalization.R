...
########################Below is the whole BIMQ Function**************************************************************
...
if (length(args)!=3) {
  stop("provide inputfile, probe annotation and outputfile names", call.=FALSE)
} 

inputfile = args[1]
probeannotfile = args[2]
outputfile = args[3]

dat0=read.csv.sql(inputfile) ;
probeAnnotation21kdatMethUsed=read.csv(probeannotfile)
...
write.table(datMethUsedNormalized,file=outputfile,sep=",",row.names=T)

print (datMethUsedNormalized)